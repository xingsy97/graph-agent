...
import type { TaskRun, WorkspaceChange, WorkspaceReport } from "@graph-agent/domain";

const execFileAsync = promisify(execFile);
const ignored = new Set([".git", ".next", "node_modules", "dist", "coverage", ".turbo"]);
...
class WorkspaceService {
  ...
  async report(run: TaskRun): Promise<WorkspaceReport> {
    ...
    for (const [file, fingerprint] of after) {
      ...
      else if (previous.size !== fingerprint.size || previous.modified !== fingerprint.modified) changes.push({ path: file, kind: "modified" });
    }
    for (const file of before.keys()) if (!after.has(file)) changes.push({ path: file, kind: "deleted" });
    const gitBranch = await this.branch(run.workspacePath);
    const verificationPattern = /test|verify|validation|build|lint|quality|review/i;
    return {
      workspacePath: run.workspacePath, ...(gitBranch ? { gitBranch } : {}), changes: changes.slice(0, 250),
      completedOutputs: run.nodes.filter((node) => node.result).map((node) => ({ nodeId: node.id, title: node.title, result: node.result! })),
      verifications: run.nodes.filter((node) => verificationPattern.test(`${node.title} ${node.task}`)).map((node) => ({ nodeId: node.id, title: node.title, status: node.status, ...(node.result ? { result: node.result } : {}) })),
    ...
    while (queue.length && files.size < 5_000) {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) queue.push(absolute);
          ...
          if (info) files.set(path.relative(root, absolute).replaceAll("\\", "/"), { size: info.size, modified: info.mtimeMs });
        }
      }
    }
    return files;
  ...
  private async branch(root: string): Promise<string | undefined> {
    try { return (await execFileAsync("git", ["branch", "--show-current"], { cwd: root, timeout: 5_000 })).stdout.trim() || undefined; } catch { return undefined; }
  }
}

declare global { var graphAgentWorkspaceService: WorkspaceService | undefined; }
export const workspaceService = globalThis.graphAgentWorkspaceService ??= new WorkspaceService();
