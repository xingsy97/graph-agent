...
export async function POST(request: Request): Promise<NextResponse> {
  const body = await request.json() as { task?: unknown; workspacePath?: unknown; speed?: unknown; workflow?: unknown };
  if (typeof body.task !== "string" || body.task.trim().length < 3 || body.task.length > 4_000) {
  ...
  const run = store.create(body.task.trim(), workspacePath, { speed, workflow });
